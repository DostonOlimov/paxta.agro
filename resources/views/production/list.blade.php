		...
		<div class="page-header">
			<ol class="breadcrumb">
				<li class="breadcrumb-item">
					<i class="fe fe-life-buoy mr-1"></i>&nbsp Ishlab chiqarish turlari
				</li>
			</ol>
		</div>
								...
								<?php $i=1;?>
								 @foreach($crops as $crop)
									<tr>
										...
										<td>{{ $crop->name }}</td>
										<td>
											<a href="{!! url ('/production/list/edit/'.$crop->id) !!}"> <button type="button" class="btn btn-round btn-success">{{ trans('app.Edit')}}</button></a>

											<a url="{!! url('/production/list/delete/'.$crop->id)!!}" class="sa-warning"> <button type="button" class="btn btn-round btn-danger dgr">{{ trans('app.Delete')}}</button></a>
										</td>
									</tr>
								<?php $i++; ?>
								@endforeach
