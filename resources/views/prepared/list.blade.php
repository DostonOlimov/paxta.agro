		...
		<div class="page-header">
			<ol class="breadcrumb">
				<li class="breadcrumb-item">
					<i class="fe fe-life-buoy mr-1"></i>&nbsp {{trans('message.Ishlab chiqaruvchi zavodlar')}}
				</li>
			</ol>
		</div>
        @if(session('message'))
								...
								<?php $i=1;?>
								 @foreach($companies as $company)
									<tr>
										...
										<td>{{ $company->kod }}</td>
										<td>{{ $company->tara}}</td>
										<td>{{ __("message." . optional($company->region)->name) }}</td>
										<td>
											<a href="{!! url ('/prepared/list/edit/'.$company->id) !!}"> <button type="button" class="btn btn-round btn-success">{{ trans('app.Edit')}}</button></a>

											<a url="{!! url('/prepared/list/delete/'.$company->id)!!}" class="sa-warning"> <button type="button" class="btn btn-round btn-danger dgr">{{ trans('app.Delete')}}</button></a>
										</td>
									</tr>
								<?php $i++; ?>
								@endforeach
