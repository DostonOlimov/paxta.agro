...
@section('content')
                ...
                <div class="col-md-12 grid-margin">
                    <div class="row">
                        <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                            ...
                            <h4 class="font-weight-normal text-light mb-0">&nbsp
                                {{ auth()->user()->name . ' ' . auth()->user()->lastname }}
                                {{ trans('message.tizimga muvaffaqiyatli kirdingiz.') }} </h4>
                        </div>
    ...
    <div class="scores empty">
        ...
        <div class="empty-inside">
            <div class="row">
                <div class="col-sm-4">
                    <select class="w-100 form-control state_of_country custom-select" name="app_type_selector"
                        id="app_type_selector">

                        <option value="">{{ trans('message.Barcha arizalar') }}</option>
                        <option value="3" @if ($app_type_selector && $app_type_selector == 3) selected="selected" @endif>
                            {{ trans('message.Jarayon to\'liq yakunlanmagan arizalar') }} </option>
                        <option value="2" @if ($app_type_selector && $app_type_selector == 2) selected="selected" @endif>
                            {{ trans('message.Sertifikat berilgan arizalar') }} </option>
                        <option value="0" @if (!is_null($app_type_selector) && $app_type_selector == 0) selected="selected" @endif>
                            {{ trans('message.Nomuvofiqligi uchun sertifikat berilmagan arizalar') }}</option>
                        <option value="1" @if ($app_type_selector && $app_type_selector == 1) selected="selected" @endif>
                            {{ trans('message.Boshqa sabablarga ko\'ra sertifikat berilmagan arizalar') }}</option>
                    </select>
                </div>
                <div class="col-sm-8">
                        ...
                        <div class="date {{ $from && $till ? 'open' : '' }}">
                            <form class="input-filter pr-0 mr-0">
                                <input class="form-control fc-datepicker from input-filter" name="from"
                                    ...
                                    @if (!empty($from)) value="{{ $from }}" @endif /> <span
                                    style="color:white">{{ trans('message.dan') }}</span>
                                <input class="form-control fc-datepicker till input-filter" name="till"
                                    placeholder="dd-mm-yyyy" autocomplete="off" required="required"
                                    @if (!empty($till)) value="{{ $till }}" @endif /> <span
                                    style="color:white">{{ trans('message.gacha') }}</span>
                                @if ($from && $till)
                                ...
                                @else
                                    <button type='submit'
                                        class="btn btn-primary  filter-button">{{ trans('message.filterlash') }}
                                    </button>
                                @endif
                            </form>
                        </div>
                    ...
                    @if (!empty($states))
                        @foreach ($states as $state)
                            <option value="{{ $state->id }}"
                                @if ($city && $city == $state->id) selected="selected" @endif> {{ $state->name }}
                            </option>
                        @endforeach
                    @endif
        ...
        <div class="workblocks__info">
            <div class="workblocks__info__row lineUp" style="animation-delay: 0.5s;">
                ...
                <div class="workblocks__info__row__duration">
                    <a href="#" rel="noopener noreferrer">{{ trans('message.Arizalar soni') }}</a>
                </div>
            ...
            @foreach ($crops as $item)
                <div class="workblocks__info__row lineUp" style="animation-delay: 0.25s;">
                    <div class="workblocks__info__row__color workblocks__info__row__color__secondary"></div>
                    <div class="workblocks__info__row__date">
                        <a href="{{ url('full-report?till=' . $till . '&from=' . $from . '&city=' . $city . '&crop=' . $item->id . '&app_type_selector=' . $app_type_selector) }}"
                            target="_blank" rel="noopener noreferrer">{{ $loop->iteration }}</a>
                    </div>
                    <div class="workblocks__info__row__theme">
                        <a href="{{ url('full-report?till=' . $till . '&from=' . $from . '&city=' . $city . '&crop=' . $item->id . '&app_type_selector=' . $app_type_selector) }}"
                    ...
                    <div class="workblocks__info__row__duration">
                        <a href="{{ url('full-report?till=' . $till . '&from=' . $from . '&city=' . $city . '&crop=' . $item->id . '&app_type_selector=' . $app_type_selector) }}"
                            target="_blank" rel="noopener noreferrer">{{ $item->applications_count }}
                            {{ trans('message.ta') }}</a>
                    </div>
                    <div class="workblocks__info__row__score">
                        <a href="{{ url('full-report?till=' . $till . '&from=' . $from . '&city=' . $city . '&crop=' . $item->id . '&app_type_selector=' . $app_type_selector) }}"
                            target="_blank" rel="noopener noreferrer">{{ round($sum_amount / 1000, 3) }}</a>
                    </div>
                    <div class="workblocks__info__row__score">
                        <a href="{{ url('full-report?till=' . $till . '&from=' . $from . '&city=' . $city . '&crop=' . $item->id . '&app_type_selector=' . $app_type_selector) }}"
                            target="_blank" rel="noopener noreferrer"> {{ trans('message.tonna') }} </a>
                    </div>
                </div>
            @endforeach
        </div>
                ...
                <select class="w-100 form-control state_of_country custom-select" name="crop" id="crop">
                    ...
                    @if (!empty($crop_names))
                        @foreach ($crop_names as $state)
                            <option value="{{ $state->id }}"
                                @if ($crop && $crop == $state->id) selected="selected" @endif> {{ $state->name }}
                            </option>
                        @endforeach
                    @endif
            ...
            @foreach ($app_states as $app_state)
                <div class="workblocks__info__row lineUp" style="animation-delay: 0.25s">
                    <div class="workblocks__info__row__percentage">
                        <a href="{{ url('full-report?till=' . $till . '&from=' . $from . '&city=' . $app_state->id . '&crop=' . $crop . '&app_type_selector=' . $app_type_selector) }}"
                            target="_blank"
                            rel="noopener noreferrer">{{ round((100 * $app_state->application_count) / $app_states->sum('application_count')) }}%</a>
                    </div>
                    <div class="workblocks__info__row__theme">
                        <a href="{{ url('full-report?till=' . $till . '&from=' . $from . '&city=' . $app_state->id . '&crop=' . $crop . '&app_type_selector=' . $app_type_selector) }}"
                            target="_blank" rel="noopener noreferrer">{{ $app_state->name }}</a>
                    </div>
                    ...
                    <div class="workblocks__info__row__duration">
                        <a href="{{ url('full-report?till=' . $till . '&from=' . $from . '&city=' . $app_state->id . '&crop=' . $crop . '&app_type_selector=' . $app_type_selector) }}"
                            target="_blank" rel="noopener noreferrer">{{ $app_state->application_count }}
                            {{ trans('message.ta') }}</a>
                    </div>
                </div>
            @endforeach
