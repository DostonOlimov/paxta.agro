								...
								<?php $i=1;?>
								 @foreach($types as $type)
									<tr>
										...
										<td>
											<a href="{!! url ('/indicator/list/edit/'.$type->id) !!}"> <button type="button" class="btn btn-round btn-success">{{ trans('app.Edit')}}</button></a>

											<a url="{!! url('/indicator/list/delete/'.$type->id)!!}" class="sa-warning"> <button type="button" class="btn btn-round btn-danger dgr">{{ trans('app.Delete')}}</button></a>
										</td>
									</tr>
								<?php $i++; ?>
								@endforeach
